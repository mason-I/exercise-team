...
const GetAllActivitiesInputSchema = z.object({
    ...
    outputFormat: z.enum(["summary", "raw", "baseline"]).optional().default("summary").describe("Return format: summary text, raw JSON, or baseline-ready JSON"),
    pretty: z.boolean().optional().default(true).describe("Pretty-print JSON for raw/baseline output")
});
...
function buildBaselineActivity(activity: any) {
    ...
    const averageSpeed = typeof activity.average_speed === "number" ? activity.average_speed : null; // m/s
    ...
    return {
        ...
        elapsed_time_sec: typeof activity.elapsed_time === "number" ? activity.elapsed_time : null,
        total_elevation_gain_m: typeof activity.total_elevation_gain === "number" ? activity.total_elevation_gain : null,
        ...
        average_heartrate: typeof activity.average_heartrate === "number" ? activity.average_heartrate : null,
...
export const getAllActivities = {
    ...
    execute: async (input: GetAllActivitiesInput) => {
        ...
        try {
            ...
            if (input.outputFormat === "raw") {
                const body = input.pretty ? JSON.stringify(resultsToReturn, null, 2) : JSON.stringify(resultsToReturn);
                return { content: [{ type: "text" as const, text: body }] };
            }

            if (input.outputFormat === "baseline") {
                const baselineActivities = resultsToReturn.map(activity => buildBaselineActivity(activity));
                const body = input.pretty ? JSON.stringify(baselineActivities, null, 2) : JSON.stringify(baselineActivities);
                return { content: [{ type: "text" as const, text: body }] };
            }
